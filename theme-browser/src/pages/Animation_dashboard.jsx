...
const Animation_dashboard = () => {
  ...
  try {
    const res = await axios.get(`${import.meta.env.VITE_API_URL}/api/uploads/category/animation`);
        ...
        <div className="relative ml-4 cursor-pointer">
          ...
          {showUploads && (
            <div className="absolute right-0 mt-2 w-40 bg-gray-800 text-white rounded-lg shadow-lg z-10">
              {['Home', 'Dashboard', 'Upload Your Work'].map((item) => (
                <p
                  key={item}
                  className="p-3 hover:bg-gray-700 cursor-pointer"
                  onClick={() => {
                    if (item === 'Upload Your Work') {
...
export default Animation_dashboard;